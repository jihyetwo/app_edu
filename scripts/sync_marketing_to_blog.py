...
마케팅 콘텐츠(00-기획서.md / 00-기획안.md) → 네이버 블로그 & 앱 게시판 자동 배선 파이프라인.

기능:
  python3 scripts/sync_marketing_to_blog.py --status
  python3 scripts/sync_marketing_to_blog.py --sync <folder_name>
  python3 scripts/sync_marketing_to_blog.py --sync-all
  python3 scripts/sync_marketing_to_blog.py --watch
  python3 scripts/sync_marketing_to_blog.py --copy-blog <folder_name>
  python3 scripts/sync_marketing_to_blog.py --copy-app <folder_name>
  python3 scripts/sync_marketing_to_blog.py --copy-tags <folder_name>
"""
...
def process_campaign(campaign, force=False):
    ...
    if not force and os.path.exists(blog_path) and os.path.exists(app_path):
        print(f"⏩ [이미 생성됨] {folder_name} (강제 갱신은 --force 플래그 사용)")
        return True

    print(f"⚙️  [변환 중] {folder_name} ...")
    with open(plan_file, "r", encoding="utf-8", errors="ignore") as fp:
        raw_plan = fp.read()
    meta = extract_metadata(raw_plan)

    # 1. 01_블로그.md 및 02_앱게시판.md 생성
    ...
    # 2. repo output 에 먼저 안전하게 저장 (항상 성공)
    ...
    # 2. 원본 EDU 마케팅 폴더 및 다채널 폴더에 쓰기 시도 (권한 허용 시)
    ...
    try:
        ...
        print(f"   ℹ️  [안내] 외부 소스 폴더({item_path}) 쓰기 권한이 제한되어 repo_dest에 우선 안전하게 저장되었습니다.")
    ...
    print(f"   · 워크스페이스 저장: {repo_dest}/ (01_블로그.md, 02_앱게시판.md)")
    if written_to_source:
        print(f"   · 마케팅 소스 저장: {blog_path}")
    print(f"   · 메인 키워드: [{blog_info['main_keyword']}] | 태그: {len(blog_info['tags'])}개 | 본문: {len(blog_content)}자")
    return True
...
def show_status():
    ...
    print("=" * 80)
    print(f"{'분류':<12} | {'캠페인 폴더':<32} | {'기획서':<6} | {'블로그':<6} | {'앱게시판':<6}")
    print("-" * 80)
    ...
    for c in campaigns:
        ...
        if c["has_blog"] and c["has_app"]:
            ...
            if c["plan_file"]:
                pending_count += 1
        print(f"{c['category']:<12} | {c['folder_name'][:30]:<32} | {p_mark:<6} | {b_mark:<6} | {a_mark:<6}")
...
def watch_mode(interval=5):
    """폴더 감시 모드: 새로운 마케팅 기획서가 생기면 자동으로 블로그/앱게시판 생성"""
    print("👀 [감시 모드 가동] /Users/name/Desktop/Cluade/EDU 폴더를 감시 중입니다... (종료: Ctrl+C)")
    seen_mtimes = {}
...
def get_blog_file(campaign):
    p1 = os.path.join(campaign["path"], "01_블로그.md")
    if os.path.exists(p1):
        return p1
    p2 = os.path.join(REPO_OUTPUT, campaign["folder_name"], "01_블로그.md")
    if os.path.exists(p2):
        return p2
    return None

def get_app_file(campaign):
    p1 = os.path.join(campaign["path"], "02_앱게시판.md")
    if os.path.exists(p1):
        return p1
    p2 = os.path.join(REPO_OUTPUT, campaign["folder_name"], "02_앱게시판.md")
    if os.path.exists(p2):
        return p2
    return None
...
def main():
    ...
    parser.add_argument("--sync-all", action="store_true", help="미생성된 모든 캠페인 일괄 생성")
    parser.add_argument("--force", action="store_true", help="이미 생성된 파일도 강제 덮어쓰기")
    parser.add_argument("--watch", action="store_true", help="실시간 폴더 감시 모드 실행")
    ...
    if args.sync_all:
        campaigns = get_all_campaigns()
        print(f"🚀 총 {len(campaigns)}개 캠페인 일괄 변환을 시작합니다...")
        success = 0
        for c in campaigns:
            if c["plan_file"]:
    ...
    if args.sync:
        ...
        if not c:
            ...
            return
        process_campaign(c, force=args.force or True)
        return
    ...
    if args.copy_blog:
        ...
        if copy_to_clipboard(text):
            print(f"📋 [복사 완료] '{c['folder_name']}' 블로그 마크다운이 클립보드에 복사되었습니다!")
            print("   👉 네이버 블로그 스마트에디터 ONE 또는 마크다운 에디터에 붙여넣기(Cmd+V)하세요.")
        return
    ...
    if args.copy_app:
        ...
        with open(app_file, "r", encoding="utf-8") as fp:
            text = fp.read()
        if copy_to_clipboard(text):
            print(f"📋 [복사 완료] '{c['folder_name']}' 앱 게시판 원문이 클립보드에 복사되었습니다!")
        return
